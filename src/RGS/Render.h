...
namespace RGS
{
	...
	struct Program
	{
		bool EnableDoubleSided = false;//是否两面都要渲染
	...
	private:
		...
		static BoundingBox GetBoundingBox(const Vec4(&fragCoords)[3], const int width, const int height);//得到包围盒
		static bool IsInsideTriangle(float(&weights)[3]);//判断是不是在三角形内
		static void CaculateWeights(float(&screenWeights)[3], float(&weights)[3], const Vec4(&fragCoords)[3], const Vec2& sreenPoint);
		static bool IsBackFacing(const Vec4& a, const Vec4& b, const Vec4& c);//判断与相机之间的朝向关系
		static bool PassDepthTest(const float writeDepth, const float fDepth, const DepthFuncType depthFunc);//深度测试
			...
			const uint32_t height)
		{
		 ...
		 static void CaculateNdcPos(varying_t(&varyings)[RGS_MAX_VARYINGS], const int vertexNum)//齐次除法
		 {
		 ...
		 static void CaculateFragPos(varyings_t(&varyings)[RGS_MAX_VARYINGS], 
			 const int vertexNum,const float width,const float height)//视口变换
		 {
			 for (int i = 0; i < vertexNum; i++)
			 {
			 ...
			 const varyings_t& varyings,const uniforms_t& uniforms)
		 {
			 ...
			 if (program.EnableWriteDepth)//写入深度
			 {
		 ...
		 template<typename vertex_t,typename uniforms_t,typename varyings_t>
		 static void RasterizeTriangle(Framebuffer& framebuffer,//光栅化
			 ...
			 const varyings_t(&varyings)[3],const uniforms_t& uniforms)
		 {
			 ...
			 BoundingBox bBox = GetBoundingBox(fragCoords, framebuffer.GetWidth(), framebuffer.GetHeight());//得到包围盒

			 for (int y = bBox.MinY; y <= bBox.MaxY; y++)//光栅化
			 {
	...
	public:
		...
		const Triangle<vertex_t>& triangle,const uniforms_t& uniforms)
		{
			...
			int vertexNum = Clip(varyings);//裁剪
			
			CaculateNdcPos(varyings, vertexNum);//映射(-1,1)范围
			int fwidth = framebuffer.GetWidth();
			int fheight = framebuffer.GetHeight();
			CaculateFragPos(varyings, vertexNum, (float)fwidth, (float)fheight);//视口变化(width,height)
			//Varyings 就是是变换后的数据
