...
namespace RGS
{
    ...
    bool Renderer::IsInsideTriangle(float(&weights)[3])//三个权值都大于0则这个点在三角形内
    {
        return weights[0] >= -EPSILON && weights[1] >= -EPSILON && weights[2] >= -EPSILON;
    }
    void Renderer::CaculateWeights(float(&screenWeights)[3], float(&weights)[3], const Vec4(&fragCoords)[3], const Vec2& sreenPoint)//该点能否用三角的两条边线性表示，存入权值
    {
        Vec2 ab = fragCoords[1] - fragCoords[0];
        Vec2 ac = fragCoords[2] - fragCoords[0];
        Vec2 ap = sreenPoint - fragCoords[0];
    ...
    bool Renderer::IsBackFacing(const Vec4& a, const Vec4& b, const Vec4& c)//是否是背面
    {
        float signedArea = a.X * b.Y - a.Y * b.X + b.X * c.Y - b.Y * c.X + c.X * a.Y - c.Y * a.X;
        return signedArea <= 0;
    }
    bool Renderer::PassDepthTest(const float writeDepth, const float fDepth, const DepthFuncType depthFunc)//深度测试
    {
        switch (depthFunc)
        {
